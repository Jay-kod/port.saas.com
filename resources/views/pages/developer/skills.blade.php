...
<div class="space-y-8 max-w-5xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button wire:click="openCreateModal('Backend')" type="button" class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold text-xs transition-all shadow-lg shadow-emerald-950/50 flex items-center gap-2 cursor-pointer shrink-0">
    ...
    @if($skills->isEmpty())
        <div class="glass-card rounded-3xl p-12 text-center space-y-4">
            ...
            <button wire:click="openCreateModal('Backend')" class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold text-xs transition-all shadow-md inline-flex items-center gap-2 cursor-pointer">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
            ...
            @foreach($groupedSkills as $categoryName => $catSkills)
                <div class="glass-card rounded-3xl p-6 sm:p-7 border border-white/5 space-y-4">
                    <div class="flex items-center justify-between border-b border-white/5 pb-3">
                        ...
                        <button wire:click="openCreateModal('{{ $categoryName }}')" class="text-xs text-emerald-400 hover:text-emerald-300 font-semibold flex items-center gap-1">
                            ...
                            <div class="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-emerald-500/40 transition-all flex flex-col justify-between space-y-2 group">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                                        <button wire:click="openEditModal({{ $skill->id }})" class="p-1 text-slate-400 hover:text-white" title="Edit">
                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </button>
                                        <button wire:click="deleteSkill({{ $skill->id }})" class="p-1 text-slate-400 hover:text-rose-400" title="Delete">
