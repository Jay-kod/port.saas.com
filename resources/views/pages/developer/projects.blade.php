                ...
                <div class="glass-card glass-card-hover rounded-3xl p-6 flex flex-col justify-between space-y-5 border border-white/5">
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div class="flex items-center gap-2">
                                @if($project->is_featured)
                                    <span class="px-2 py-0.5 rounded-md text-[10px] font-bold font-mono bg-yellow-500/10 text-yellow-400 border border-yellow-500/30">
                                        FEATURED
                                    </span>
                                @endif
                            ...
                            <div class="flex items-center gap-1.5">
                                <button wire:click="openEditModal({{ $project->id }})" class="p-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-white transition-colors" title="Edit">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </button>
                                <button wire:click="deleteProject({{ $project->id }})" wire:confirm="Are you sure you want to delete this project?" class="p-1.5 rounded-lg bg-slate-900 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 transition-colors" title="Delete">
                    ...
                    <div class="pt-3 border-t border-white/5 flex items-center justify-between text-xs">
                        <div class="flex items-center gap-3 font-mono">
                            @if($project->repo_url)
                                <a href="{{ $project->repo_url }}" target="_blank" class="text-slate-400 hover:text-white flex items-center gap-1">
                            ...
                            @endif
                            @if($project->live_url)
                                <a href="{{ $project->live_url }}" target="_blank" class="text-emerald-400 hover:text-emerald-300 flex items-center gap-1 font-semibold">
                                    <span>Demo &rarr;</span>
                                </a>
                            @endif
                        </div>
                        ...
                        @if(Auth::user()?->profile?->slug)
                            <a href="{{ url('/' . Auth::user()->profile->slug . '/projects/' . $project->slug) }}" target="_blank" class="text-[11px] text-slate-500 hover:text-slate-300">
                                View &nearr;
                            </a>
                        @endif
                    </div>
