    ...
    <header class="fixed top-0 inset-x-0 h-16 bg-green-700 text-white z-50 flex items-center px-4 shadow-md border-b-4 border-yellow-400">
        ...
        <div class="ml-auto flex items-center space-x-4">
            ...
            <form method="POST" action="{{ route('filament.admin.auth.logout') }}" class="inline">
                @csrf
                <button type="submit" class="text-xs bg-green-800 hover:bg-green-900 text-yellow-300 px-3 py-1.5 rounded font-semibold transition-colors">
                    Sign Out
                </button>
            </form>
        </div>
    ...
    <div class="flex h-screen pt-16">
        ...
        >
            <nav class="flex flex-col p-2 space-y-1">
                ...
                <!-- Super Admin Link -->
                <a href="{{ route('super-admin.dashboard') }}" class="flex items-center p-2 rounded-md hover:bg-red-50 hover:text-red-700 group transition-colors">
                    ...
                    <span class="ml-3 font-medium text-red-600 whitespace-nowrap" x-show="sidebarOpen">Super Admin</span>
                </a>
                @endif
            </nav>
        </aside>
